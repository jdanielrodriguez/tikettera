...
export class ClientesService {
  ...
  async getAll(): Promise<any> {
    ...
    const url = `${this.basePath}/api/clientes`;
    try {
  ...
  async getAllFilter(data: any): Promise<any> {
    ...
    const url = `${this.basePath}/api/filter/${data.id}/clientes/${data.estado}${filter}`;
    this.mySesion.reloadToken();
    try {
      const response = await this.http.get(url, { headers: this.mySesion.headers })
        .toPromise();
  ...
  async delete(id: number): Promise<any> {
    const url = `${this.basePath}/api/clientes/${id}`;
    try {
  ...
  async update(form: any): Promise<any> {
    this.mySesion.reloadToken();
    const url = `${this.basePath}/api/clientes/${form.id}`;
    try {
